...
def load_whisper_modules(model_id: str, device=None):
...
def load_wav2vec2_modules(model_id: str, device=None):
...
def transcribe_video_whisper(
    ...
    # Determine device and handle MPS compatibility
    if torch.cuda.is_available():
        device = "cuda"
        ...
        try:
            ...
            logging.warning(f"MPS initialization failed, falling back to CPU: {str(e)}")
            device = "cpu"
    ...
    if not using_cpu_fallback:
        try:
            ...
            logging.warning(f"Failed to move model to {device}, falling back to CPU: {str(e)}")
            device = "cpu"
            using_cpu_fallback = True
            model = model.to(device)
    ...
    for i, audio_chunk in enumerate(chunk_directory):
        ...
        try:
            ...
            if device == "mps" and not using_cpu_fallback:
                try:
                    # Convert to float32 first
                    input_features = input_features.to(torch.float32)
                    
                    # Move to CPU, make contiguous, then back to MPS
                    input_features = input_features.cpu().contiguous()
                    ...
                    logging.warning(f"MPS operation failed, falling back to CPU: {str(e)}")
                    device = "cpu"
                    using_cpu_fallback = True
                    model = model.to(device)
            ...
            logging.warning(f"Error transcribing chunk {i} in {clip_name}")
...
def transcribe_dir(
    ...
    Args:
        input_dir (str): Directory containing video files to transcribe
        chunk_length (int, optional): Length of audio chunks in seconds. Defaults to 30.
        model_id (str, optional): HuggingFace model ID. Defaults to "openai/whisper-base.en".
        basic_spelling (bool, optional): Use basic spell checker. Defaults to False.
        move_comp (bool, optional): Move completed files. Defaults to False.
        join_text (bool, optional): Join text files. Defaults to False.
        print_memory_usage (bool, optional): Print memory usage. Defaults to False.
        verbose (bool, optional): Print verbose output. Defaults to False.
        device (str, optional): Device to use for inference ('cuda', 'mps', or 'cpu'). Defaults to None.
    """
    ...
    # Pass device to model loading functions
    processor, model = (
        load_whisper_modules(model_id, device=device)
        if _is_whisper
        else load_wav2vec2_modules(model_id, device=device)
    )
