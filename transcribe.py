    ...
    for i, audio_chunk in enumerate(chunk_directory):
        ...
        try:
            ...
            if device == "mps" and not using_cpu_fallback:
                try:
                    # Add additional error handling for MPS
                    ...
                    with torch.no_grad():
                        predicted_ids = model.generate(
                            ...
                            length_penalty=1.0  # Normalize length
                        )
            ...
            # Improved decoding with better error handling
            try:
            ...
            logging.error(f"Error transcribing chunk {i} in {clip_name}: {str(e)}")
            this_transc = ""
